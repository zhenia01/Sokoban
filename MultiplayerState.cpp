...
MultiplayerState::MultiplayerState(StateManager& manager, const AssetManager<Textures::Game::ID, sf::Texture>& textures) : _manager(manager), _gameTextures(textures){
	_type = StateType::Multiplayer;
}
...
void MultiplayerState::draw(sf::RenderTarget& target, sf::RenderStates states) {
	if (_progress[1] == '1') {
...
void MultiplayerState::update(const sf::Time&) {
}
...
void MultiplayerState::init(const sf::RenderWindow& window, const AssetManager<Textures::Menu::ID, sf::Texture>& textures) {
	
	std::ifstream in("Assets/savemp.save");
	...
	_newGame.setCallBack([&]() mutable {
		_progress = " 0 0 0 0 0";
		std::ofstream out("Assets/savemp.save");
		out << _progress << '\n';
		for (int i = 0; i < 5; i++) {
			out << '0' << "\n";
		}
	...
	_1.setCallBack([&]() {
		...
		_manager.addState(StateType::Playing, std::shared_ptr<PlayingState>(new PlayingState(_manager, 1, true, _gameTextures)));
		_manager.init(window, textures);
		});
	_2.setCallBack([&]() {
		_manager.switchTo(StateType::Playing);
		_manager.addState(StateType::Playing, std::shared_ptr<PlayingState>(new PlayingState(_manager, 2, true, _gameTextures)));
		_manager.init(window, textures);
	...
	_3.setCallBack([&]() {
		_manager.switchTo(StateType::Playing);
		_manager.addState(StateType::Playing, std::shared_ptr<PlayingState>(new PlayingState(_manager, 3, true, _gameTextures)));
		_manager.init(window, textures);

		});
	_4.setCallBack([&]() {
		_manager.switchTo(StateType::Playing);
		_manager.addState(StateType::Playing, std::shared_ptr<PlayingState>(new PlayingState(_manager, 4, true, _gameTextures)));
		_manager.init(window, textures);

		});
	_5.setCallBack([&]() {
		_manager.switchTo(StateType::Playing);
		_manager.addState(StateType::Playing, std::shared_ptr<PlayingState>(new PlayingState(_manager, 5, true, _gameTextures)));
		_manager.init(window, textures);
